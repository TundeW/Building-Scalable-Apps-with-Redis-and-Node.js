...
var socketConnection = function socketConnection(socket){ 
    ...
    socket.on('AddChat', function(chat){ 
        var newChat = models.Chat(chat.message, chat.room, models.User(socket.handshake.user.id, socket.handshake.user. displayName, socket.handshake.user.provider));
    ...
    socket.on('AddRoom', function(r){ 
        ...
        removeAllRooms(socket, function(){
            if (room !== '') {
                ...
                socket.broadcast.to(room).emit('AddUser', models.User(socket.handshake.user.id, socket.handshake.user. displayName, socket.handshake.user.provider));
...
var removeFromRoom = function removeFromRoom(socket, room){ 
    ...
    socket.broadcast.to(room).emit('RemoveUser', models.User(socket.request.user.id, socket.request.user. displayName, socket.request.user.provider));
};
