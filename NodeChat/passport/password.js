var crypto = require('crypto'), scmp = require('scmp'), config = require('../config');
...
var passwordCreate = function passwordCreate(password, cb){ 
    crypto.randomBytes(config.crypto.randomSize, function(err, salt){
        ...
        crypto.pbkdf2(password, salt.toString('base64'), config.crypto.workFactor, config.crypto.keylen, 'sha1', function(err, key){
...
var passwordCheck = function passwordCheck(password, derivedPassword, salt, work, cb){
    crypto.pbkdf2(password, salt, work, config.crypto.keylen, 'sha1', function(err, key){
